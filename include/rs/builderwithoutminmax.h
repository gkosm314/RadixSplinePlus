...
class BuilderWithoutMinMax {
 public:
  ...
  void AddKey(KeyType key) {
    if (curr_num_keys_ == 0) {
      // Initalize min, max and prev keys using the first key
      min_key_ = key;
      max_key_ = key;
      prev_key_ = key;
    ...
    max_key_ = key; // update the max_key, assuming that the keys are inserted in an increasing order
 ...
 private:
  ...
  void PossiblyAddKeyToRadixTable(KeyType key, const uint32_t curr_index) {
  ...
  void ConstructRadixTable() {
    ...
    // Iterate over the spline points and construct the radix table
    for(size_t current_index = 0; current_index < spline_points_.size(); current_index++) 
    PossiblyAddKeyToRadixTable(spline_points_[current_index].x, current_index);
  }
